...
leftMissoffsets = {'60', '110'}  -- you probably won't need these if it's for the opponent
downMissoffsets = {'30', '-50'}  --
upMissoffsets = {'10', '30'}    --
rightMissoffsets = {'-30', '40'} --
...
upAltoffsets = {'0', '0'}
rightAltoffsets = {'0', '0'}
idleAltoffsets = {'-70', '20'}
smackoffsets = {'-70', '20'}

-- change all of these to the name of the animation in your character's xml file
idle_xml_name = 'Sanic idle'
left_xml_name = 'Sanic L pose'
down_xml_name = 'Sanic D pose'
up_xml_name = 'Sanic U pose'
right_xml_name = 'Sanic R pose'

left_miss_xml_name = 'Sanic L pose'   -- you probably won't need these if it's for the opponent
down_miss_xml_name = 'Sanic D pose'   --
up_miss_xml_name = 'Sanic U pose'       --
right_miss_xml_name = 'Sanic R pose' --

left_alt_xml_name = 'left alt'
down_alt_xml_name = 'down alt'
up_alt_xml_name = 'up alt'
right_alt_xml_name = 'right alt'
idle_alt_xml_name = 'Sanic Smack0015'
smack_xml_name = 'Sanic Smack'
...
function onCreate()
	...
	addAnimationByPrefix(name_of_character, 'idle-alt', idle_alt_xml_name, 1, false);
	addAnimationByPrefix(name_of_character, 'smack', smack_xml_name, 1, false);
...
function onEvent(name, value1, value2)
	if name == 'Griddy' then --events BS
		...
		if gritty == 1 then --he appears
			setPropertyLuaSprite(name_of_character, 'alpha', 1)
		elseif gritty == 3 then --the slap
			runTimer('slapperino',0.55);
		end
	end
end

function onTimerCompleted(t,l,ll)
	if t == 'slapperino' then
		objectPlayAnimation(name_of_character, 'smack', false)
		setProperty(name_of_character .. '.offset.x', smackoffsets[1])
		setProperty(name_of_character .. '.offset.y', smackoffsets[2])
		idlealt = true
	end
end
